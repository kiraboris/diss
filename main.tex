...
If the rotor is a symmetric or asymmetric molecule, there are groups of parameters which depend more on $J$, on $K_a$ and on $K_c$.\todo{Details here!}
...
\begin{itemize}
	...
	\item Transitions with $\Delta J = +1$, $0$ and $-1$ are called R-branch, Q-branch and P-branch respectively. Transitions belonging to different branches have different dependency on model parameters.\todo{Details here!}
	\item Transitions caused by different dipole moment components are called a-type, b-type and c-type transitions. 
\end{itemize}


It is reasonable\todo{Why?} to respect the structure of the spectrum during its assignment. The overall direction of assignment is from lower frequencies to higher frequencies, which correspond with going from lower $J$ values to higher $J$ values. 
